...
class Staffattendance extends Admin_Controller
{
    ...
    {
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/staffattendance/bioreport');
        $this->load->model('bio_model');
        $doctorlist         = $this->bio_model->getBioIDs();
        $data['doctorlist'] = $doctorlist;
        $select             = 'staff_bio_attendance.*';
        $join               = array(
            'JOIN user_bio ON user_bio.bio_id = staff_bio_attendance.bio_id',
        );
        $where              = array();
        $bioid           = $this->input->post('doctor');
        $disable_option     = false;
        $data['doctor_select']  = $bioid;
        if (!empty($doctorid)) {
            $where = array('staff_bio_attendance.bio_id =' . $bioid);
        }
        $table_name = "staff_bio_attendance";
        ...
        if (empty($search_type)) {
            $search_type = "";
            $resultlist  = $this->bio_model->getReport($select, $join, $table_name, $where);
        } else {
            $search_table  = "staff_bio_attendance";
            $search_column = "push_time";
            $resultlist    = $this->bio_model->searchReport($select, $join, $table_name, $search_type, $search_table, $search_column, $where);
        }
        ...
        $data["resultlist"]  = $resultlist;
        $this->load->view('layout/header');
        $this->load->view('admin/bio/bio_report.php', $data);
        $this->load->view('layout/footer');
    }
