...
class Staff extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data_insert = array(
                ...
                'opd_commission'       => $this->input->post('opd_com'),
                'ot_commission'       => $this->input->post('ot_com'),
                'pathology_commission'       => $this->input->post('pathology_com'),
                'emg_commission'       => $this->input->post('emg_com'),
                'ipd_commission'       => $this->input->post('ipd_com'),
                'is_active'            => 1,
            );
    ...
    public function bulkPaid()
    {
        ...
        foreach( $ids as $key=>$id){
            $bulkData[]=array(
                'id'=>$id,
                'comission_status'=>'paid',
                'paid_by'=>$this->session->userdata('hospitaladmin')['id'],
            );
        }
       
        if($this->db->update_batch('monthly_comission', $bulkData, 'id')){return true;}else{return false;}
    
    }

    public function printPaidInvoice($id)
    {
        $data['invoice_detail']=$invoice_detail=$this->staff_model->paidPrintInvoice($id);
        $html  = $this->load->view('admin/staff/paid_print_invoie',$data, true);
        // Load pdf library
